...
class GeocirclesDAOTest(
    jdbi: Jdbi,
    private val fixtures: Fixtures
) {
    private val puzzleDAO: PuzzleDAO = jdbi.onDemand()

    private val puzzleOne = Puzzle(Game.GEOCIRCLES, 123, null)
    private val puzzleTwo = Puzzle(Game.FLAGLE, 124, null)

    private val geocirclesDAO: PinpointDAO = jdbi.onDemand()
    ...
    fun `can insert a result`() {
        val result = geocirclesDAO.insertResult(
    ...
    @Test
    fun `can retrieve geocircles result for user and puzzle`() {
        insertResult(puzzle = puzzleOne)

        val result = geocirclesDAO.resultForUserOnPuzzle(fixtures.zach.id, puzzleOne)
    ...
    fun `only retrieves results for given user`() {
        ...
        val jackieResult = geocirclesDAO.resultForUserOnPuzzle(fixtures.jackie.id, puzzleOne)

        assertThat(jackieResult).isNotNull()
        assertThat(jackieResult!!.userId).isEqualTo(fixtures.jackie.id)

        val zachResult = geocirclesDAO.resultForUserOnPuzzle(fixtures.zach.id, puzzleOne)
    ...
    fun `can list all results for a puzzle`() {
        ...
        val results = geocirclesDAO.resultsForPuzzle(puzzleOne).toList()
    ...
    fun `can list all results for all puzzles`() {
        ...
        val results = geocirclesDAO.allResultsStream().toList()
    ...
    fun `can query by time range for a single user`() {
        ...
        val results = geocirclesDAO.resultsForUserInTimeRange(
            userId = fixtures.zach.id,
    ...
    private fun insertResult(
        ...
        return geocirclesDAO.insertResult(userId, puzzle, score, shareText)
    }
}
