...
username=$(find .. -name "*.env" -exec grep -m 1 "DB_USER" {} \; | cut -d '=' -f 2)
password=$(find .. -name "*.env" -exec grep -m 1 "DB_PASSWORD" {} \; | cut -d '=' -f 2)
db_name=$(find .. -name "*.env" -exec grep -m 1 "DB_NAME" {} \; | cut -d '=' -f 2)
